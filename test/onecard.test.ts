...
describe("One Card Game Logic", () => {
  ...
  it("selects one card bot defenses when under attack", () => {
    const state = baseState(2);
  ...
  it("saves one card attack cards when a safer card is playable", () => {
    const state = baseState(2);
    state.players[0] = { ...state.players[0]!, kind: "bot", botDifficulty: "medium" };
    state.stageState = {
      discardPile: ["5S"],
  ...
  it("selects one card color joker defense with a chosen suit", () => {
    const state = baseState(2);
