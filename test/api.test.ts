...
describe("HTTP API", () => {
  it("lists built-in games and creates a lobby room", async () => {
    ...
    expect(gamesBody.games.map((game) => game.gameId)).toEqual(["card-demo", "gomoku", "onecard"]);
    ...
    expect(filteredBody.games.map((game) => game.gameId)).toEqual(["card-demo", "gomoku", "onecard"]);
