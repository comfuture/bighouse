...
describe("HTTP API", () => {
  it("lists built-in games and creates a lobby room", async () => {
    ...
    expect(gamesBody.games.map((game) => game.gameId)).toEqual(["chess", "gomoku", "onecard", "tank-battle"]);
    ...
    expect(filteredBody.games.map((game) => game.gameId)).toEqual(["chess", "gomoku", "onecard", "tank-battle"]);
